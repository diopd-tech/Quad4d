# Note de passation — Quad4D / Click'n Fly
...
**rapprochements dangereux et des blocages**. Il n'est pas « débrayé en
attendant mieux » : il a été retiré du code parce qu'il ne tenait pas.

C'est écrit ici pour que personne ne le réimplémente en croyant à un oubli. La
sécurité repose sur des méthodes calculées avant le vol, prévisibles et
validables en simulation.
...
Deux réserves, toutes deux écrites en tête de `spatial_deconfliction.py` :
...
- **Le forçage de départ** (`transit_shortfalls`, `begin_show`,
  ...
  l'IHM le nomme puis propose, au bout de 15 s, de lancer quand même. Le code
  est écrit et relu, il n'a pas volé. *(Supprimer cette ligne une fois validé.)*
- **Le nombre maximal de drones simultanés** : [à compléter — combien de drones
  réels ont volé ensemble, et combien seulement en simulation].
...
- **Les seuils de batterie viennent de l'airframe.** Ils sont lus dans la
  ...
  ne bouge. C'est voulu — l'IHM et le drone ne peuvent pas diverger — mais ça
  surprend.
...
3. Chercher la cause du comportement du 110, en commençant par le test au sol
   ci-dessus — sans présumer que c'est la bonne piste.
4. Reprendre la déconfliction sur plusieurs cycles de boucle, pas seulement le
   premier.

Le reste — indicateur de lien datalink, autonomie restante, son, land sélectif —
est dans [TODO.md](TODO.md) et relève du confort, pas de la sécurité.
